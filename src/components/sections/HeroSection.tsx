...
const HeroSection = () => {
  ...
  return (
    <section className="relative isolate min-h-[90vh] md:min-h-screen flex items-center bg-hero overflow-x-hidden overflow-y-hidden pt-20">
      {/* Premium Background Effects */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {/* Main gold glow (clamped) */}
        <motion.div
          className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[min(800px,90vw)] h-[min(600px,70vh)] bg-primary/8 rounded-full blur-[80px] md:blur-[120px]"
          animate={{
            scale: [1, 1.1, 1],
            opacity: [0.6, 0.8, 0.6],
          }}
        ...
        {/* Secondary glows (clamped to viewport) */}
        <motion.div
          className="absolute top-1/4 left-[max(6%,12px)] w-[min(384px,55vw)] h-[min(384px,55vw)] bg-primary/10 rounded-full blur-3xl"
          animate={{
            x: [0, 30, 0],
            y: [0, -20, 0],
          }}
        ...
        <motion.div
          className="absolute bottom-1/4 right-[max(6%,12px)] w-[min(320px,50vw)] h-[min(320px,50vw)] bg-primary/5 rounded-full blur-3xl"
          animate={{
            x: [0, -20, 0],
            y: [0, 30, 0],
          }}
          ...
          >
            <a href={bookingLink} target="_blank" rel="noopener noreferrer">
              <Button
                variant="hero"
                size="xl"
                className="shadow-[0_0_40px_-10px_hsl(var(--primary)/0.6)] hover:shadow-[0_0_50px_-5px_hsl(var(--primary)/0.8)] transition-all duration-500"
              >
                {t("hero.cta")}
